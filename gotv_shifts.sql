  ...
  , shifts as (
   ...
   , event_state as (
    select vanid, eventid, eventshiftid, estate as state from (
      select case WHEN ev.createdbycommitteeid = 56351 or (eventname like '%NV%' and eventname not like '%NVRD%') THEN 'NV'
            WHEN ev.createdbycommitteeid = 62658 or eventname like '%FL%' THEN 'FL'
              WHEN ev.createdbycommitteeid = 56350 or eventname like '%IA%' THEN 'IA'
              WHEN ev.createdbycommitteeid = 56354 or eventname like '%PA%' THEN 'PA'
              WHEN ev.createdbycommitteeid = 60315 or eventname like '%VA%' THEN 'VA'
              WHEN ev.createdbycommitteeid = 64624 or eventname like '%AZ%' THEN 'AZ'
              WHEN ev.createdbycommitteeid = 56352 or eventname like '%NH%' THEN 'NH'
              WHEN ev.createdbycommitteeid = 57273 or eventname like '%NC%' THEN 'NC'
              WHEN ev.createdbycommitteeid = 62659 or eventname like '%MI%' THEN 'MI'
              WHEN ev.createdbycommitteeid = 76878 or eventname like '%ME%' THEN 'ME'
              WHEN ev.createdbycommitteeid = 62660 or eventname like '%WI%' THEN 'WI'
              when ev.createdbycommitteeid = 85292 or eventname like '%National NextGen Organizing%' then 'DIST'
...
select distinct
    ...
    , case when sg.state is not null then right(sg.turf,2) when sg.state is null and mrr.state is not null then 'OOS' when sg.state is null and mrr.state is null then 'Unturfed' else null end as turf 
    ...
    , case when sh.status ilike 'Sched%' then 'Scheduled' when sh.status ilike 'Left Msg' then 'Scheduled' else sh.status end as status
    , sh.dateupdated
		, sh.shift_time
    , date(sh.shift_time) as shift_day
    , case when sh.status ilike 'Sched%' or sh.status ilike 'Conf%' then 'Y' else null end as shift_open
    , case when sh.status ilike 'Sched%' then 'Scheduled' when sh.status ilike 'Conf%' then 'Confirmed' else null end as open_status
...
order by eventstate asc, turf asc, gotv_event asc, shift_type asc, sh.shift_time asc, sh.status asc, signupdate desc, dateupdated desc
